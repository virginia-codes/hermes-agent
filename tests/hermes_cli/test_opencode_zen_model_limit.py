...
def test_opencode_zen_lists_all_models_while_other_providers_remain_capped(monkeypatch):
    ...
    nvidia_models = [f"nvidia-model-{i}" for i in range(57)]
    ...
    nvidia = next(p for p in providers if p["slug"] == "nvidia")

    assert opencode_zen["models"] == zen_models
    assert opencode_zen["total_models"] == len(zen_models)
    assert nvidia["models"] == nvidia_models[:50]
    assert nvidia["total_models"] == len(nvidia_models)
